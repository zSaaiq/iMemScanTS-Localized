...
class VMOneKeyCtrl: UITableViewController {
    ...
    func setupViews() {
        ...
        indexs = []
        VMOneKeyTool.setup()
        let data = VMOneKeyTool.allRecords(.record)
        let arr = data.filter { (model) -> Bool in
            let m = model as! VMOneKeyModel
            return m.gid == gid
        }
        dataArray.append(contentsOf: arr)
...
//        ]
    ...
    @objc func longPressAction(_ gesture: UIGestureRecognizer?) {
        ...
        UIAlertController.showAlert_holder(title, message: "注意后面两个字带有[定时]两个字\n代表需要循环执行脚本", holder: model.name, buttonTitle: "修改", handler: { [self] text in
            model.name = text!
    ...
    override func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        ...
        VMOneKeyTool.exchange(sourceIndexPath.row, index: destinationIndexPath.row, type: .record)
    }
    
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            
            UIAlertController.showAlert5("提示", message: "确定删除该脚本?", buttonTitle: "删除", isdefault: false) { [self] in
                let model = dataArray[indexPath.row] as!VMOneKeyModel
